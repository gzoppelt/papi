...
def get_camera_from_id(camid):
    """Return the camera for the given camera ID."""
...
class BaseCamera(object):
    ...
    def __init__(self):
        self.camid = None  # to be desfined by subclasses
...
class PicCamera(BaseCamera):
    """Raspberry Pi camera module handler class."""
    def __init__(self):
        super(PicCamera, self).__init__()
        self.camid = 'pi'
...
class FakeCamera(BaseCamera):
    ...
    def capture(self):
        """Capture a (fake) picture. This really cpies a stock jpeg."""
        filename = self.get_new_photo_filename()
...
# load the cameras global with the list of avalable cameras
...
def get_camras():
    """Return a list of avalable cameras."""
...
def get_photo(camid, filename):
    """Return a photo. Photos are in jpeg format,
